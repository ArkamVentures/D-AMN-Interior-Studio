...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  const { globalSettings, contact } = useData();
  ...
  if (globalSettings.maintenanceMode) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#050505] text-white p-6 text-center">
        <span className="text-xs font-semibold tracking-[0.3em] text-[#C9A227] uppercase mb-4">D-AMN Aluminium</span>
        <h1 className="text-4xl font-serif font-bold mb-4">Under Maintenance</h1>
        <p className="text-gray-400 max-w-md mx-auto text-sm leading-relaxed mb-6">
          We are currently polishing our website to serve you better. We'll be back shortly!
        </p>
        <a href={`tel:${contact.phone}`} className="text-sm text-[#C9A227] font-semibold hover:underline">
  ...
  return (
    <div className="min-h-screen flex flex-col bg-warm-gray dark:bg-dark-bg text-primary dark:text-gray-100 transition-colors duration-300">
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="md:hidden bg-white dark:bg-dark-card border-b border-gray-100 dark:border-white/5"
            >
              <div className="px-4 py-4 space-y-3">
                {navLinks.map((link) => (
                  <NavLink
                    key={link.to}
                    to={link.to}
                    className="block text-sm font-semibold tracking-wider uppercase text-primary/70 dark:text-white/70"
                  >
                    {link.label}
                  </NavLink>
                ))}
      ...
      {/* Footer */}
      <footer className="bg-[#0a0a0a] text-white/75 pt-16 pb-8 border-t border-white/5 transition-colors">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center flex flex-col items-center">
          ...
          <div className="flex flex-col items-center gap-3 mb-10 text-sm text-gray-300">
            <a href={`tel:${contact.phone}`} className="flex items-center gap-2 hover:text-accent transition-colors duration-300">
              <Phone className="w-4 h-4 text-accent" />
              {contact.phone}
            </a>
            <a href={globalSettings.googleMaps} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-accent transition-colors duration-300">
          ...
          {/* Social Icons */}
          <div className="mb-4">
            <p className="text-xs uppercase tracking-widest text-[#C9A227] font-semibold mb-6">Follow us for daily inspiration</p>
            <div className="flex justify-center items-center gap-4">
              <a href={globalSettings.instagram} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:text-[#E1306C] hover:bg-[#E1306C]/10 hover:scale-110 hover:shadow-[0_0_15px_rgba(225,48,108,0.5)] transition-all duration-300" aria-label="Instagram">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
              </a>
              <a href={globalSettings.facebook} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:text-[#1877F2] hover:bg-[#1877F2]/10 hover:scale-110 hover:shadow-[0_0_15px_rgba(24,119,242,0.5)] transition-all duration-300" aria-label="Facebook">
              ...
              <a href={globalSettings.tiktok} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:text-white hover:bg-black hover:border hover:border-white/20 hover:scale-110 hover:shadow-[0_0_15px_rgba(255,255,255,0.3)] transition-all duration-300" aria-label="TikTok">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/></svg>
              </a>
              <a href={globalSettings.whatsappLink} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:text-[#25D366] hover:bg-[#25D366]/10 hover:scale-110 hover:shadow-[0_0_15px_rgba(37,211,102,0.5)] transition-all duration-300" aria-label="WhatsApp">
              ...
              <a href={globalSettings.googleMaps} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:text-accent hover:bg-accent/10 hover:scale-110 hover:shadow-[0_0_15px_rgba(201,162,39,0.5)] transition-all duration-300" aria-label="Google Maps">
          ...
          <div className="border-t border-white/5 pt-8 text-xs text-gray-600 w-full">
            <p>&copy; {new Date().getFullYear()} {globalSettings.footerCopyright}</p>
          </div>
        </div>
      </footer>
      ...
      <a
        href={globalSettings.whatsappLink}
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 bg-[#25D366] text-white font-semibold rounded-full shadow-[0_4px_15px_rgba(37,211,102,0.4)] hover:bg-[#20ba5a] hover:scale-105 active:scale-95 transition-all duration-300"
      >
